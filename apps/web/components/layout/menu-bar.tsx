					...
					{({ close }) => (
						<>
							...
							<Item
								...
								onSelect={() =>
									run(close, () =>
										setPageSetup({ ...activeSetup, pageless: !activeSetup.pageless }, 'document'),
									)
								}
							>
								Pageless
					...
					{({ close }) => (
						<>
							<Item
								...
								onSelect={() => run(close, () => setImageDialogOpen(true))}
							>
								Gambar…
							</Item>
							<Item
							...
							<Item
								...
								disabled={!activeId}
								onSelect={() =>
									run(close, () => editor?.chain().focus().insertToc({ listKind: 'isi' }).run())
								}
							>
								Daftar isi
							</Item>
							<Item
								...
								disabled={!activeId}
								onSelect={() =>
									run(close, () => editor?.chain().focus().insertToc({ listKind: 'gambar' }).run())
								}
							>
								Daftar gambar
							</Item>
							<Item
								...
								disabled={!activeId}
								onSelect={() =>
									run(close, () => editor?.chain().focus().insertToc({ listKind: 'tabel' }).run())
								}
							>
								Daftar tabel
							</Item>
										...
										<Submenu label="Ukuran huruf" icon={<ALargeSmall className="h-4 w-4" />}>
											{() => (
												<>
													...
													<Item
														icon={<Minus className="h-4 w-4" />}
														onSelect={() => stepFontSize(editor, -1)}
													>
														Perkecil
													</Item>
												</>
