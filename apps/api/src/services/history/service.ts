import type { AnalysisResultData, JobStatus } from '@writer-hub/shared'
...
function resultOf(row: DetailRow): HistoryDetail['result'] {
	if (row.request.feature === 'grammar') {
		if (!row.grammar) return null
		return {
			original_text: row.grammar.original_text,
			corrected_text: row.grammar.corrected_text,
			suggestions: row.grammar.suggestions ?? [],
			scores: row.grammar.scores,
			writing_quality: row.grammar.writing_quality,
			quality_label: row.grammar.quality_label,
		}
	}
	return row.analysis?.result ?? null
}
		...
		try {
			...
			const detail: HistoryDetail = {
				...
				summary: summarize({
					...
					grammarScore: row.grammar?.writing_quality ?? null,
					suggestionCount: row.grammar?.suggestions?.length ?? null,
					analysisChangeCount: changeCountOf(row.analysis?.result),
					analysisLabel: labelOf(row.analysis?.result),
					analysisScore: scoreOf(row.analysis?.result),
				}),
