		...
		parameters: {
			...
			properties: {
				...
				fit: {
					...
					enum: ['page', 'embed'],
					description:
						'"page" for a design that occupies one whole sheet edge to edge - a flyer, poster, one-pager, or the cover, chapter title page or back cover of a book. It is placed alone on its own page and bleeds past the margins. The frame you are given is ALREADY exactly the sheet, in the document\'s current size and orientation - both are stated in the editor context you receive, so never assume landscape. You are NOT writing a web page that displays a sheet; your markup IS the sheet. Give the root element width:100%; height:100% and never a fixed pixel size - not even the numbers get_page_setup reports, which are for judging proportions, not for writing into CSS. Do not style <body> as a viewport that centres a page inside it (no min-height:100vh, no display:flex with align-items:center on body), and do not paint a "desk" colour behind the design: each of those leaves the artwork floating with dead margins around it. Size in % or em, never vh/vw. Anything taller than the sheet is CUT OFF at the page edge, never scaled down. "embed" (default) is a small piece that flows inline with the surrounding text and uses the "height" argument.',
				},
				height: {
					type: 'number',
					description:
